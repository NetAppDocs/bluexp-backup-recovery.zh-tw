...
*附註：*如果您要還原的磁碟區備份檔案位於歸檔儲存設備（從ONTAP 版本號9.10.1開始）中、還原作業將需要較長的時間、而且會產生成本。此外、目的地叢集也必須執行ONTAP 支援磁碟區還原的版本為《支援使用者支援者支援者支援者支援對象》（更新版本）、適用於檔案還原的版本為9.11.1、以及適用於Google歸檔的版本為9.12.1。
...
[cols="50,50"]
|===
| 備份檔案位置目的地工作環境ifdef：：AWS [] | Amazon S3 


| AWS內部部署的不全系統endif::AWS [] ifdef:azure[] Cloud Volumes ONTAP ONTAP | Azure Blob 


| Azure內部部署的系統中的資料：：azure[] ifdef：：Cloud Volumes ONTAP ONTAP GCP[] | Google Cloud Storage 


| 在Google內部部署中的系統資訊：Cloud Volumes ONTAP ONTAP GCP[] | NetApp StorageGRID 
|===
在瀏覽與還原中、連接器可安裝在下列位置：
...
如果您要還原的磁碟區備份檔案位於歸檔儲存設備（ONTAP 從版本號9.10.1開始提供）、還原作業將需要較長的時間、並會產生額外成本。請注意、目的地叢集也必須執行ONTAP 支援磁碟區還原功能的版本為《支援使用者支援者支援者支援者支援對象》（更新版本）、適用於檔案還原的版本為《支援者支援者支援者支援者支援者支援者支援者支援者支援者
