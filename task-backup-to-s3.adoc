...
+
--
* 在Cloud Manager中新增目的地AWS帳戶認證資料
* 將「S3：PuttBucketPolicy」權限新增至IAM角色、以提供Cloud Manager權限
...
+
如果您想要使用不同的AWS帳戶進行備份、則必須在Cloud Manager中新增目的地AWS帳戶認證、並將「S3：PuttketPolicy」權限新增至IAM角色、以提供Cloud Manager權限。
