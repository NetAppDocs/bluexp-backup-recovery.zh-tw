...
* 您在Cloud Volumes ONTAP AWS中執行的是版本號為9、7P5或更新版本。
...
* 提供具有權限的BlueXP Connector的IAM角色包括來自最新版本的S3權限 https://mysupport.netapp.com/site/info/cloud-manager-policies["BlueXP原則"^]。
...
+
--
* 在BluXP中新增目的地AWS帳戶認證資料
* 將「S3：PuttBucketPolicy」權限新增至IAM角色、為BlueXP提供權限
...
需要 AWS 權限:: 提供BlueXP權限的IAM角色必須包含最新版本的S3權限 https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions-aws.html["BlueXP原則"^]。
+
--
以下是原則的特定權限：
...
+
如果您想要使用不同的AWS帳戶進行備份、則必須在BlueXP中新增目的地AWS帳戶認證、並將「S3：PuttketPolicy」權限新增至IAM角色、以便為BlueXP提供權限。
