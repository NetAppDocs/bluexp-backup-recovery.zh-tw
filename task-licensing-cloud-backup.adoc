...
* 「 CVO Edge Cache 」套件的功能與「 CVO Professional 」套件相同、但也支援 https://docs.netapp.com/us-en/cloud-manager-file-cache/concept-gfc.html["BlueXP 邊緣快取"^] 服務：您有權在 Cloud Volumes ONTAP 系統上為每 3 TiB 的已配置容量部署一個 BlueXP 邊緣快取邊緣系統。此選項可透過 Azure 和 Google Marketplaces 取得、但無法讓您備份內部部署的 ONTAP 資料。
