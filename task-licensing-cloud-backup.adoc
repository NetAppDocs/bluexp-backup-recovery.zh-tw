...
* 「 CVO Edge Cache 」套件的功能與「 CVO Professional 」套件相同、但也支援 https://["BlueXP 邊緣快取"^] 服務：您有權在 Cloud Volumes ONTAP 系統上為每 3 TiB 的已配置容量部署一個 BlueXP 邊緣快取邊緣系統。此選項可透過 Azure 和 Google Marketplaces 取得、但無法讓您備份內部部署的 ONTAP 資料。
...
* AWS ： https://["如需價格詳細資料、請前往BlueXP Marketplace產品"^]。
...
* Azure ： https://["如需價格詳細資料、請前往BlueXP Marketplace產品"^]。
...
* GCP ： https://["如需價格詳細資料、請前往BlueXP Marketplace產品"^]。
...
使用AWS時、可從取得兩份年度合約 https://["AWS Marketplace頁面"^] 適用於Cloud Volumes ONTAP 內部和內部部署ONTAP 的不全系統。提供1年、2年或3年期限：

* 「雲端備份」計畫、可讓您備份Cloud Volumes ONTAP 內部部署ONTAP 的支援資料。
+
如果您要使用此選項、請從「市場」頁面設定您的訂閱、然後再進行設定 https://["將訂閱與AWS認證資料建立關聯"^]。請注意、您也需要使用這項年度合約訂閱來支付 Cloud Volumes ONTAP 系統的費用、因為您只能在 BlueXP 中指派一個有效訂閱給 AWS 認證。
...
+
請參閱 https://["介紹授權主題Cloud Volumes ONTAP"^] 以深入瞭解此授權選項。
...
或者、如果您沒有使用未指派的 Cloud Volumes ONTAP 節點型授權、您可以將其轉換為具有相同美元對等和相同到期日的 BlueXP 備份與還原授權。 https://["如需詳細資料、請前往此處"^]。
...
如果授權期限即將到期、或授權容量已達到上限、您將會在備份UI中收到通知。此狀態也會出現在 BlueXP 數位電子錢包頁面和中 https://["通知"]。
