...
從雲端備份 UI 和 API 啟動的使用者起始備份和還原作業、以及系統起始的工作（例如持續排程備份作業）、都可以在執行 ONTAP 9.13.0 或更新版本的 Cloud Volumes ONTAP 系統的 * 工作監控 * 標籤中找到。
...
您可以在修改後、將「工作監控」主頁的內容下載為報告。Cloud Backup會產生並下載.CSV檔案、您可以視需要檢閱並傳送給其他群組。CSV檔案最多包含10、000列資料。
...
若要接收 Cloud Backup 電子郵件警示、您必須選取通知嚴重性類型「重大」和「錯誤」。
