...
每個BlueXP backup and recoveryAPI 的文件可從以下位置取得 https://docs.netapp.com/us-en/bluexp-automation/cbs/overview.html["BlueXP自動化，用於BlueXP backup and recovery"^]。
...
NOTE: 如果您使用服務帳戶，則應使用服務存取令牌而不是使用者令牌。 「client_id」的值（「Mu0V1ywgYteI6w1MbD15fKfVIUrNXGWC」）是固定值，無法變更。在這種情況下，請按照此處的說明進行操作： https://docs.netapp.com/us-en/bluexp-automation/platform/create_service_token.html["建立服務存取權令牌"^] 。
