...
+
如果您使用的是私有Google存取或私有服務連線、請確定DNS伺服器已設定為將storage.googleapis.com指向正確的內部（私有）IP位址。
...
+
請依照Google指示設定這些私人存取選項。請確定您的DNS伺服器已設定為將www.googleapis.com和storage.googleapis.com指向正確的內部（私有）IP位址。
