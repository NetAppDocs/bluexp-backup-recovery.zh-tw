...
* 您已探索內部部署叢集、並將其新增為藍圖XP中的工作環境。請參閱 https://["探索 ONTAP 叢集"^] 以取得詳細資料。
+
...
+
** 透過連接埠 443 連接到 BlueXP 備份與恢復服務、以及 Google Cloud 儲存設備的 HTTPS 連線 (https://["請參閱端點清單"^]）
** 透過連接埠443連線至ONTAP 您的SURF叢 集管理LIF的HTTPS連線


. 在您打算部署Connector的子網路上啟用私有Google Access（或Private Service Connect）。 https://["私有 Google 存取"^] 或 https://["私有服務連線"^] 如果ONTAP 您從某個叢集直接連線至VPC、而且想要連接器與Google Cloud Storage之間的通訊保持在虛擬私有網路（*私有*連線）中、就需要使用此功能。
+
...
.步驟
. 在中 https://["Google Cloud Console"^]請移至*角色*頁面。
...
. 前往 https://["GCP 儲存設定"^] 並建立服務帳戶的存取金鑰：
+
...
+
** 如需 BlueXP 備份與還原 PAYGO 授權、您需要訂閱 https://["Google Marketplace 提供的 NetApp BlueXP 產品"^]。BlueXP 備份與還原的帳單是透過此訂閱完成。
...
+
您可以從內部部署系統建立備份、並在所有地區建立Google Cloud Storage https://["支援的地方 Cloud Volumes ONTAP"^]。您可以指定在設定服務時儲存備份的區域。
