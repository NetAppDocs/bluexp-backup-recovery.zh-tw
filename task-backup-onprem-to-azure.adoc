...
* https://["深入瞭解連接器"^]
* https://["在Azure中安裝Connector"^]
* https://["在內部環境中安裝連接器"^]
* https://["在Azure政府區域安裝Connector"^]
+
...
+
** 透過連接埠 443 連接到 BlueXP 備份與恢復服務、以及到 Blob 物件儲存設備的 HTTPS 連線 (https://["請參閱端點清單"^]）
** 透過連接埠443連線至ONTAP 您的SURF叢 集管理LIF的HTTPS連線
** 為了讓 BlueXP 備份與還原搜尋與還原功能正常運作、連接埠 1433 必須開啟、才能在 Connector 與 Azure Synapse SQL 服務之間進行通訊。
** Azure和Azure政府部署需要額外的傳入安全性群組規則。請參閱 https://["Azure 中的 Connector 規則"^] 以取得詳細資料。
...
您可以在所有地區、從內部部署系統建立備份到Azure Blob https://["支援的地方 Cloud Volumes ONTAP"^]（包括Azure政府區域）。您可以指定在設定服務時儲存備份的區域。
...
+
** 如需 BlueXP 備份與還原 PAYGO 授權、您需要訂閱 https://["Azure Marketplace 提供的 NetApp BlueXP 產品"^]。BlueXP 備份與還原的帳單是透過此訂閱完成。
...
. 您可以在啟動精靈中使用自己的自訂管理金鑰進行資料加密、而非使用預設的Microsoft管理加密金鑰。在此情況下、您必須擁有Azure訂閱、Key Vault名稱及金鑰。 https://["瞭解如何使用您自己的金鑰"^]。
. 如果您想要透過公用網際網路從內部資料中心連線至vnet、可以在啟動精靈中設定Azure私有端點。在這種情況下、您需要知道此連線的vnet和子網路。 https://["如需使用私有端點的詳細資訊、請參閱"^]。
...
.. 您也可以選擇是否要設定Azure私有端點。 https://["如需使用私有端點的詳細資訊、請參閱"^]。
+
image:screenshot_backup_onprem_azure_networking.png["快照顯示將磁碟區從ONTAP 某個系統備份到Azure Blob儲存設備時的網路詳細資料。"]
