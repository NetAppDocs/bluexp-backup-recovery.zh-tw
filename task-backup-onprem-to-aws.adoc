...
* https://["深入瞭解連接器"^]
* https://["在AWS中安裝Connector"^]
* https://["在內部環境中安裝連接器"^]
* https://["在AWS GovCloud區域安裝Connector"^]
+
...
+
** 透過連接埠 443 連接到 BlueXP 備份與還原服務、以及 S3 物件儲存設備的 HTTPS 連線 (https://["請參閱端點清單"^]）
** 透過連接埠443連線至ONTAP 您的SURF叢 集管理LIF的HTTPS連線
** AWS和AWS GovCloud部署需要額外的傳入和傳出安全群組規則。請參閱 https://["AWS 中 Connector 的規則"^] 以取得詳細資料。
...
* 叢集間生命體必須與_IPspacer_建立關聯、ONTAP 以便連接物件儲存設備。 https://["深入瞭解 IPspaces"^]。
+
...
* DNS伺服器必須已針對磁碟區所在的儲存VM進行設定。瞭解如何操作 https://["設定SVM的DNS服務"^]。
...
+
** 如需 BlueXP 備份與還原 PAYGO 授權、您需要訂閱 https://["AWS Marketplace 提供的 NetApp BlueXP 產品"^]。BlueXP 備份與還原的帳單是透過此訂閱完成。
...
+
您可以在所有地區、從內部部署系統建立備份到Amazon S3 https://["支援的地方 Cloud Volumes ONTAP"^]；包括AWS GovCloud區域。您可以指定在設定服務時儲存備份的區域。
...
.步驟
. 確認下列S3權限（從最新版本開始） https://["BlueXP原則"^]）是IAM角色的一部分、可為Connector提供權限。如果沒有、請參閱 https://["AWS文件：編輯IAM原則"]。
+
[source, json]
----
{
...
----
+
請參閱 https://["AWS 文件：建立角色、將權限委派給 IAM 使用者"^] 以取得詳細資料。
...
如果您想要使用自己的客戶管理金鑰進行資料加密、而不是使用預設金鑰、則必須先設定加密託管金鑰、才能啟動 BlueXP 備份與還原精靈。 https://["瞭解如何使用您自己的金鑰"^]。
...
. 使用Amazon VPC主控台或命令列建立介面端點組態。 https://["如需使用AWS Private Link for Amazon S3的詳細資訊、請參閱"^]。
...
. 從VPC S3端點取得憑證。您的做法是 https://["登入裝載BlueXP Connector的VM"^] 並執行下列命令。輸入端點的DNS名稱時、請在開頭加入「pucket」、取代「*」：
+
[source, text]
----
...
.. 儲存備份的AWS區域。
.. 無論您是使用預設的Amazon S3加密金鑰、還是從AWS帳戶選擇自己的客戶管理金鑰、都能管理資料的加密。 (https://["瞭解如何使用您自己的金鑰"]）。
+
...
.. 您也可以選擇是否要使用先前設定的AWS Private Link。 https://["如需使用AWS Private Link for Amazon S3的詳細資訊、請參閱"^]。
+
