...
https://["如需詳細資料、請參閱DataLock和勒索軟體保護部落格"^]。

此功能無法為來源磁碟區提供保護、只能用於這些來源磁碟區的備份。使用NetApp https://["不再是不一樣的Cloud Insights Cloud Secure"^]或部分 https://["提供來自VMware的反勒索軟體保護ONTAP"^] 保護來源磁碟區。
...
*** S3：GetObjectVersion
+
https://["檢視原則的完整Json格式、您可以在其中複製及貼上所需的權限"^]。
