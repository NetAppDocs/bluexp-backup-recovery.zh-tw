...
這兩項功能都能保護您的備份檔案、因此當勒索軟體攻擊來源資料時、您永遠都能擁有有效的備份檔案來還原資料。此外、也有助於滿足某些法規要求、在某些情況下、備份必須鎖定並保留一段時間。啟用 DataLock 和勒索軟體保護時、配置為 BlueXP 備份和恢復啟動一部分的雲端貯體將會啟用物件鎖定和物件版本控制。
