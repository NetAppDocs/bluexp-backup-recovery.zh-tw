...
[role="lead"]
BlueXP 備份與還原可讓您在 Azure 帳戶中建立不同於來源 Cloud Volumes ONTAP 磁碟區所在位置的備份檔案。這兩個帳戶可能不同於BlueXP Connector所在的帳戶。

只有在您需要時、才需要執行這些步驟 https://["將Cloud Volumes ONTAP 不支援的資料備份到Azure Blob儲存設備"^]。
