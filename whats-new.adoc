...
之前、您必須先建立自訂原則、才能啟動 Snapshot 、複寫或備份。從 ONTAP 9.8 P5 開始、您現在可以使用 BlueXP 備份與還原 UI 、在備份啟動程序期間建立原則。
