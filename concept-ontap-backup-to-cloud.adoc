...
請參閱 <<複寫限制,複寫限制>> 如需 BlueXP 備份與還原無法使用的複寫功能清單、請參閱。
...
* 如果您需要從已移至歸檔物件儲存區的備份檔案還原 Volume 資料、則需要向雲端供應商收取額外的每 GiB 擷取費用和每項要求費用。
...
+
** 在 StorageGRID 中、 BlueXP 會使用現有的儲存帳戶來儲存物件儲存區貯體。
