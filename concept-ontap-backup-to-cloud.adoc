...
Cloud Backup可用於沒有網際網路連線的站台（也稱為「離線」或「暗」站台）、以備份Volume資料。此組態也支援Volume與檔案還原。在這種情況下、您需要在Dark網站上部署BlueXP Connector。
...
* 您可以將Cloud Volumes ONTAP 安裝在AWS和Azure商用區域的支援資料系統備份到Amazon S3或Azure Blob。請參閱 link:task-backup-to-s3.html["將Cloud Volumes ONTAP 不支援的資料備份至Amazon S3"] 和 link:task-backup-to-azure.html["將Cloud Volumes ONTAP 無法取得的資料備份到Azure Blob"]。
